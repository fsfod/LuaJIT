...
static int ra_reflive(ASMState *as, IRIns *ir)
{
...
void asm_intrin_results(ASMState *as, IRIns *ir, AsmIntrins* intrins, IntrinsInfo* ininfo)
{
  ...
  if (intrins->outsz > 0) {
    ...
    int32_t n = intrins->outsz-1;

    for (;;) {
      lua_assert(irret->o == IR_ASMRET || irret->o == IR_ASMINS);
      results[n--] = (IRRef)(irret - as->ir);

      if (ra_hasreg(irret->r)) {
        Reg r = irret->r;
  ...
  for (; i < intrins->insz; i++) {
    ...
    /* don't evict if we were allocated correct register */
    if (rset_test(evict, r) || !ra_reflive(as, arg)) {
      evict |= RID2RSET(r);
    }
  }
  ...
  if (intrins->outsz > dynout) {
    int32_t stop = dynout;
    for (int32_t i = intrins->outsz-1; i >= stop; i--) {
      IRIns *irr = IR(results[i]);
      Reg r = intrins->out[i];

      /* Don't evict ourselves */
      if (!rset_test(as->freeset, r) && regcost_ref(as->cost[r]) != results[i]) {
