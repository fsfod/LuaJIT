...
static LJ_AINLINE void log_arenacreated(uint32_t arenaid, void * address, GCSize totalmem, uint32_t flags)
{
  SBuf *sb = &eventbuf;
...
#define nummarkermsg_id(msg) (((msg)->msgid >> 8) & 0xffff)
#define nummarkermsg_flags(msg) (((msg)->msgid >> 24) & 0xff)
static LJ_AINLINE void log_nummarker(uint32_t id, uint32_t flags, uint32_t number)
{
  SBuf *sb = &eventbuf;
  MSG_nummarker *msg = (MSG_nummarker *)sbufP(sb);
  msg->msgid = MSGID_nummarker;
  msg->msgid |= (id << 8);
  msg->msgid |= (flags << 24);
  msg->number = number;
  setsbufP(sb, sbufP(sb)+sizeof(MSG_nummarker));
  lj_buf_more(sb, 16);
}

static LJ_AINLINE MSize print_nummarker(void* msgptr)
{
  MSG_nummarker *msg = (MSG_nummarker *)msgptr;
  lua_assert(((uint8_t)msg->msgid) == MSGID_nummarker);
  printf("nummarker: id %u, flags %u, number %u\n", ((msg->msgid >> 8) & 0xffff), ((msg->msgid >> 24) & 0xff), msg->number);
  return 8;
}
...
static LJ_AINLINE void log_gcstate(uint32_t state, uint32_t prevstate, uint32_t totalmem, uint32_t hugemem, uint32_t strnum)
{
  SBuf *sb = &eventbuf;
