...
static void vmevent_loadscript(VMPrintContext *context, jit_State *J, VMEventData_LoadScript *eventdata)
{
  if (eventdata) {
    vmevent_log(context, "LOADSCRIPT(Start): name %s, isfile %s\n", eventdata->name, eventdata->code ? "false" : "true");
  } else {
    /* The Lua script has finished being loaded */
    vmevent_log(context, "LOADSCRIPT(Stop)\n");
  }
}
...
static void vmevent_callback(void *contextptr, lua_State *L, int eventid, void *eventdata)
{
  ...
  switch (event) {
    ...
    case VMEVENT_STATE_CLOSING:
      vmevent_log(context, "STATE_CLOSING\n");
