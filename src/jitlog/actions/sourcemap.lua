...
local function splitlines(s)
  ...
  for i=1, #s do
	...
	if c == carridge or (c == newline and prev ~= i) then
	  local line = ""
	  
	  if prev ~= i-1 then
		line = string.sub(s, prev+1, math.max(i-1, 1))
	  end
	  table.insert(t, line)
	  
	  prev = i
	  -- Skip over the /n following a /r
	  if c == carridge and str_byte(s, i+1) == newline then
		prev = prev + 1
	  end
	end
  end
  ...
  if prev ~= #s then
	table.insert(t, string.sub(s, prev, #s))
  end
...
local function parse_funcname(line, chunk, linenum)
  local kind
  ...
  --print(chunk, linenum, line, kind, name, libname, args)
...
function sourcemap.actions:loadscript(msg, script)
  ...
  local lines = script.source and splitlines(script.source)
  script.lines = lines
  --print("Chunk", script.name, "has lines", #lines)
  if not lines then
    return
  end
...
function sourcemap.actions:obj_proto(msg, proto)
  ...
  local lines = proto.chunk and self:get_chunklines(proto.chunk)
...
function api:get_funcname(...)
  ...
  return parse_funcname(line, chunk, linenum)
end
...
function api:get_chunklines(chunk)
  assert(type(chunk) == "string", "Expected a chunk name string")
  assert(#chunk > 0, "Expected non empty string for chunk name")
