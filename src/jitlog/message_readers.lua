...
function readers:loadscript(msg)
  ...
  if msg.isloadstart then
    info = {
      ...
      load_kind = self.vmdef.fastfuncs[msg.caller_ffid],
      protos = {}
    }
...
function readers:scriptsrc(msg)
  local chunk = msg.sourcechunk
  self:log_msg("scriptsrc", "script source chunk: length= %d", #chunk)

  local script = self.loading_script
  if not script or not chunk or #chunk == 0 then
    return
  end
  script.source = (script.source or "") .. chunk
end
...
function readers:obj_func(msg)
  ...
  if msg.ffid == 0 then
    ...
    if msg.ffid ~= 255 then
      func.fastfunc = self.vmdef.fastfuncs[msg.ffid]
    end
...
function api:decode_irins(ins)
  local irname = self.vmdef.ir
  local m1, m2 = get_irmode(ins.o, self.ir_mode)
  
  local op, op1, op2 = self.vmdef.ir[ins.o], ins.op1, ins.op2
  ...
  local irt = self.vmdef.irtypes[band(ins.t, 31 )]
...
function api:gen_irdecoders()
  local irtype = self.vmdef.irtypes
  ...
  local irfields = self.vmdef.irfields
  local ircalls = self.vmdef.ircalls
  assert(ircalls, "Missing IRCalls enum")
  assert(irfields, "Missing IRFields enum")
  assert(self.vmdef.irfpmath, "Missing FPMATH enum")
...
function gctrace:get_irconstant(irref)
  ...
  local o = self.owner.vmdef.ir[ins.o]
  local types = self.owner.vmdef.irtypes
...
function gctrace:get_consttab()
  ...
  local irname = self.owner.vmdef.ir
  local t = {}
...
function readers:trace(msg)
  ...
  local trace = {
    ...
    stoppc = msg.stoppc,
    link = msg.link,
    linktype = self.vmdef.trace_link[msg.linktype],
  ...
  if aborted then
    ...
    trace.abortreason = self.vmdef.trace_errors[msg.abortcode]
    trace.abortinfo = msg.abortinfo
    
    local abortkind = self.vmdef.terror[msg.abortcode]
    if abortkind == "NYIBC" then
      trace.abortreason = format("NYI: bytecode %s", self.vmdef.bc[trace.abortinfo])
    elseif abortkind == "NYIFFU" then
      trace.abortreason = format("NYI: fast function %s", self.vmdef.fastfuncs[trace.abortinfo])
    elseif abortkind == "NYIIR" then
      trace.abortreason = format("NYI: cannot assemble IR instruction %d", self.vmdef.ir[trace.abortinfo])
    end
...
function fbreaders:VMDef(msg)
  local vmdef = {
    ...
    fastfuncs = util.make_enum(msg:get_fastfuncs()),
...
function stacksnapshot:has_funcproto(proto)
  assert(proto, "Missing function prototype object to find")
  if self.base == -1 then
    assert(false, "stack has no base index")
  end
  local framesz = self.GC64 and 2 or 1
  ...
  while(slot >= limit) do
    ...
    if pt == proto or func and func.proto == proto then
      return true
    end
...
function stacksnapshot:printframes()
  ...
  if slot == self.slots.length-1 and self.savedpc and self.savedpc ~= 0 then
    local pt, line = self.owner:pc2proto(self.savedpc-4)
    -- We may not have all the function protos recored in log so this extra info is optional
    if pt then
      bcindex = pt:get_bcindex(self.savedpc-4)
      print(string.format("SavedPC: %s", pt:get_bclocation(bcindex)))
    else
      print(string.format("SavedPC: Unknown proto for pc address %d", self.savedpc-4))
    end
  end
...
function fbreaders:stacksnapshot(fbtbl)
  local stack = {
    owner = self,
    framesonly = fbtbl.framesonly,
    flags = fbtbl.flags,
    base = fbtbl.base,
    top =  fbtbl.top,
    vmstate = fbtbl.vmstate,
    savedpc = fbtbl.savedpc,
    GC64 = self.GC64,
  }
  stack.slots = self:read_array("TValue", fbtbl:get_slots())
  setmetatable(stack, self.msgobj_mt.stacksnapshot)

  self:log_msg("stacksnapshot", "StackSnapshot: slots = %d, base = %d", stack.slots.length, fbtbl.base)
  return stack
end
...
function readers:obj_alloc(msg, name)
  local address = addrtonum(msg.address)
  ...
  elseif location_kind == "trace_id" then
    object.trace = tonumber(msg.location)
...
readers.obj_allocstack = readers.obj_alloc
...
function api:parseheader(header)
  ...
  if vmdef then
    ...
    self.ir_mode = self.vmdef.ir_mode
  end
