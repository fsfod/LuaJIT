...
local function parselog(log, verbose, mixins)
  local result = readerlib.makereader({readerdef = reader_def, mixins = mixins or testmixins})
  if verbose then
    result.verbose = true
  end
...
it("string marker stacks", function()
  jitlog.start()
  jitlog.writemarker("marker1", 0x10000)
  jitlog.writemarker("marker2", 0x10001)
...
it("stack snapshot", function()
  jitlog.start()
  if not jitlog.write_stacksnapshot then
    return
  end
...
local perf_mixin = {
  {
    ...
    actions = {
      perf_snapshot = function(self, msg, counters, timers)
        local tab = {
          eventid = self.eventid,
          counters = util.clone(self.counters),
          counters_updated = counters ~= nil,
          timers = util.clone(self.timers),
          timers_updated = timers ~= nil,
        }     
