...
function logreader:readheader(buff, buffsize, info)
  ...
  local msgtype = band(headerstart.header, 0xff)
  if msgtype ~= 0 then
    return false, "Bad header msg type id expected 0 was "..msgtype
  end

  local headersize = headerstart.msgsize
  if headerstart.msgsize > buffsize then
    return false, string.format("Bad header size %d is larger than buffer or file size %d", headersize, buffsize)
  end

  if headerstart.vtable >= 0 or (-headerstart.vtable + 8 + 4) > buffsize then
    return false, string.format("Bad vtable offset of %d for the header", -headerstart.vtable + 8)
  end
  ...
  if info.timerfreq == 0 and string.find(info.cpumodel:lower(), "intel") then
  ...
  for i = 1, msgtype_count do
    msgnames[i] = file_typenames[i]
  end
  info.msgnames = msgnames
  info.typenames = file_typenames

  self:log_msg("header", "  MsgTypes: %s", table.concat(msgnames, ", "))

  local sizes, count = header:get_msgsizes()
  local file_msgsizes = {}
  for i = 1, count do
    file_msgsizes[i] = sizes[i-1]
  end
...
function logreader:format_time(time, isseconds)
  ...
  if isseconds then
    ...
    seconds = seconds/tonumber(self.timerfreq)
  end
  
  if seconds < 1 then
    return string.format("%0.4fms", seconds*1000)
  else
...
local function make_msgparser(file_msgsizes, dispatch, aftermsg, premsg)
  ...
  return function(self, buff, length)
    ...
    while pos < buffend do
      ...
      -- We should never see the header message type here so msgtype > 0
      if msgtype <= 0 or msgtype >= msgtype_max then
...
local function make_msghandler(reader, base, funcs, name)
...
function logreader:processheader(header)
  ...
  self.typenames = header.typenames
  ...
  for i, name in ipairs(header.msgnames) do
    ...
    local parser = msgparsers[name]
    -- We must have both a parser for the message and an action to handled the
    -- parsed message, otherwise this message type is skipped over.
    if parser and (msgreaders[name] or action) then
      dispatch[i-1] = make_msghandler(parser, msgreaders[name], action, name)
    end
  end
...
function logreader:build_msgparser(name, vtable)
  if vtable then
    local type = ffi.typeof("$ *", self.logdef.messages[name])
    return function(ptr)
      return (ffi_cast(type, ptr))
    end
  end
end
...
function lib.parsebuffer(buff, length, partial)
  ...
  local reader = lib.makereader({reader_def = default_logdef})
