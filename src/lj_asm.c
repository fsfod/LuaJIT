...
{
  AsmIntrins* intrins = lj_intrinsic_get(ctype_ctsG(J2G(as->J)), ir->op2);
  ...
  if (intrin_dynrout(intrins) && intrins->outsz > 0) {
    ir->prev = REGSP_INIT;
  }
...
static void intrins_setup(AsmIntrins *intrins, IntrinsState *info)
{
  ...
  i = intrin_dynrout(intrins) ? 1 : 0;
  lua_assert(intrins->outsz == 0 || i == 0 || reg_isdyn(info->out[0]));
  for (; i < intrins->outsz; i++) {
...
static void intrins_loadregs(ASMState *as, AsmIntrins *intrins, IntrinsState *info)
{
  ...
  if (rset_test(info->inset, RID_CONTEXT) && info->contexofs != -1) {
    ...
    emit_loadofs(as, &GPRIns, RID_CONTEXT, RID_CONTEXT, info->contexofs);
  }
  ...
  for (i = 0; i < intrins->insz; i++) {
    ...
    if (reg_isgpr(reg)) {
      if (r != RID_CONTEXT)
        emit_loadofs(as, &GPRIns, r, RID_CONTEXT, gpr * sizeof(intptr_t));
      gpr++;
    } else {
...
static void intrins_saveregs(ASMState *as, AsmIntrins *intrins, IntrinsState *info)
{
  ...
  for (i = 0; i < intrins->outsz; i++) {
    ...
    if (intrins->flags & INTRINSFLAG_SAVETOSTRUCT) {
      /* TODO: Saving output registers to struct */
      lua_assert(!reg_isvec(reg));
      offset += lj_ir_type_size[reg_irt(reg)];
    } else {
...
static void* wrap_intrins(jit_State *J, AsmIntrins *intrins, void* target, MSize targetsz)
{
  ...
  if (dynreg) {
    ...
    if (intrins->outsz > 0) {
      /* handle the dynamic output register */
...
restart:
  if ((info.modregs & ~RSET_SCRATCH) || info.contexspill) {
    spadj = lj_popcnt(as->modset & (RSET_GPR & ~RSET_SCRATCH)) * sizeof(intptr_t) +
            lj_popcnt(as->modset & (RSET_FPR & ~RSET_SCRATCH)) * FPRSAVESZ;
 ...
 */
  ...
  if (rset_test(info.modregs, info.outcontext) && intrins->outsz > 0) {
    emit_storeofs(as, &GPRIns, info.outcontext, RID_SP, CONTEXTSPILL);
  }
  ...
  if ((intrins->flags & INTRINSFLAG_CALLEDIND) == INTRINSFLAG_CALLEDIND) {
    ...
    lua_assert(asmofs == 0 || (asmofs-as->mcp) > 0 && (asmofs-as->mcp) < 0xffff);
