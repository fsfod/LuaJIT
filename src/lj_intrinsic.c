...
int lj_intrinsic_create(lua_State *L)
{
  ...
  if (!istemplate) {
    ...
    id = lj_intrinsic_template(L, tname, intrins);
  }
...
CTypeID intrinsic_toffi(CTState *cts, AsmIntrins* intrins, GCstr *opstr, CTypeID ret)
{
  ...
  if (intrin_regmode(intrins) == DYNREG_OPEXT) {
    ct->info |= intrin_getopextb(intrins) << 16;
  }
...
AsmIntrins *lj_intrinsic_fromffi(CTState *cts, CType *func, AsmIntrins *intrins)
{
  ...
  if (intrin_regmode(intrins) == DYNREG_OPEXT) {
    intrin_setopextb(intrins, (ct->info >> 16) & 0xff);
  }
  intrins->mcode = NULL;
...
static void buildffiwrapper(CTState *cts, CType *func, AsmIntrins *intrins) {
  ...
  uint32_t opcode;
  int regmode = intrin_regmode(intrins);
  ...
  */
  ...
  if (intrin_regmode(intrins) == DYNREG_FIXED) {
    ...
    intrins->wrapped = lj_intrinsic_buildwrap(cts->L, intrins, intrins->mcode, 
                                              intrins->asmsz);
  } else {
...
int lj_intrinsic_fromcdef(lua_State *L, CTypeID fid, GCstr *opstr, uint32_t imm)
{
  ...
  if (buildflags & INTRINSFLAG_EXPLICTREGS) {
    rinset = process_reglist(L, intrins, REGSET_IN, NULL, sib);
  } else {
  ...
  if (intrin_iscomm(intrins) && 
    ...
    /* Invalid register setup/kinds for commutative flag */
    return 0;
  }
  ...
  return intrinsic_toffi(cts, intrins, opstr, sib);
}
