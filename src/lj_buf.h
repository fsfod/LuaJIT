...
{
  /* Don't try to adjust a buffer pointer that hasn't been allocated */
  if (sbufB(sb) == NULL) 
    return;
  lj_mem_free(g, sbufB(sb)-sizeof(GCstr), sbufsz(sb)+sizeof(GCstr));
}
...
LJ_FUNCA GCstr * LJ_FASTCALL lj_buf_tostr_tmp(SBuf *sb);
